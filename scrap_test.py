...
from dataEda import perform_eda  # Import the EDA function
...
def read_dataset(file_path):
    try:
        ...
        app_name = re.sub(r'[^a-zA-Z0-9\s]', '', file_path.split('/')[-1].split('.')[0])  # Extract app name from file name
        return df, app_name
...
def perform_eda_and_save(file_path, app_name):
    # Perform EDA on the dataset and save it
    eda_file_path = perform_eda(file_path)
...
def main():
    ...
    if url:
        ...
        driver.quit()
        eda_file_path = perform_eda_and_save(saved_file_path, app_name)
        call_sentiment_analysis(eda_file_path)
    elif file_path:
        dataset, app_name = read_dataset(file_path)
        if dataset is not None:
            ...
            if reviews_df is not None:
                ...
                eda_file_path = perform_eda_and_save(file_name, app_name)
                call_sentiment_analysis(eda_file_path)
            else:
